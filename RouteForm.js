...
const RouteForm = ({ route, onSubmit, onCancel, countries = [] }) => {
  const [formData, setFormData] = useState({
    name: '',
    countryId: '', // Сохраняем как countryId для простоты в UI
    price_usd: '',
    duration_days: '',
    description: '',
    photo_url: '',
  ...
  useEffect(() => {
    if (route) {
      // Преобразуем route.country (ObjectId или объект) в countryId
      const countryId = route.country?._id || route.country || '';
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (validateForm()) {
      // API преобразует countryId в country для MongoDB
      onSubmit({
        ...formData,
              ...
              <Form.Group className="mb-3">
                ...
                >
                  ...
                  {countries.map(country => {
                    // Используем id из normalizeObject для совместимости
